...
const ConvertToEditable = () => {
  ...
  // const handleLanguageChange = (event, pdfId) => {
  //   const newLanguage = event.target.value;

  //   // Update the language only for the specific PDF
  //   setPdfs((prevPdfs) =>
  //     prevPdfs.map((pdf) =>
  //       pdf._id === pdfId ? { ...pdf, selectedLanguage: newLanguage } : pdf
  //     )
  //   );

  //   // Add any additional logic you want to perform when the language changes
  // };
  ...
  const fetchOutputDetails = async (pdfId) => {
    try {
      ...
      // Assuming responseData is an object with pdfId as a key
      // Assuming responseData is an array of output details
      const outputDetailsMap = {};

      // Store output details in a dictionary with pdfId as the key
      ...
      const filteredOutputDetails = Object.values(outputDetailsMap).filter(
        (output) => {
          return (
            output.fileName.includes("Editable(eng)") ||
            output.fileName.includes("Editable(hin)")
          );
        }
      );
  ...
  const handleProcess = async (pdfId, language) => {
    try {
      ...
      // Adjust the API endpoint based on the selected language
      let endpoint;
    if (language === 'English') {
      endpoint = `/api/user/processpdf/${pdfId}/${currentUser._id}`;
    } else if (language === 'Hindi') {
      endpoint = `/api/user/processpdfhindi/${pdfId}/${currentUser._id}`;
    } else {
  ...
  const handleDownload = async (outputId) => {
    try {
      ...
      const response = await fetch(`api/user/outputdetail/${outputId}`);
  ...
  function callme() {
    var timeOutPromise = new Promise(function (resolve, reject) {
      // 5 Second delay
      setTimeout(resolve, 105000, "Timeout Done");
    });
      ...
      <div className="ml-[16%] h-[100vh] overflow-hidden">
        <div className="flex justify-between p-8 items-center w-[100%]">
          <h1 className="text-2xl flex items-center">
            DASHBOARD <FaArrowRight className="mx-3" /> CONVERT TO EDITABLE MASTER COPY
          </h1>
        </div>
...
export default ConvertToEditable;